...
- **Wrap-and-clear at logical boundaries.** Claude proactively suggests `/session-wrap` (or the `/handoff` alias) at three trigger points: (a) a substantive commit-and-push checkpoint (a feat / refactor / new-tests / new-docs commit landing on a remote branch — NOT a chore/style/typo micro-fix), (b) a `[Resolution]` event that closes the loop on a previously-logged `[Friction]`, (c) end of session. Plus a fallback at ~60% context fill when no natural checkpoint has hit (practitioner consensus: quality degrades materially before the 200K limit; wrap earlier than feels necessary). After the wrap completes, Claude proposes `/clear` so the next work chunk starts with a fresh context window — the wrap has captured what mattered to memory (Open Brain + friction-log + CHANGELOG + a project-side `PENDING_TASKS.md` dumped from the in-session task list, which is otherwise wiped by `/clear`); raw conversation history was paying token-tax for context that's now persisted. Mid-session wraps use the lighter `--checkpoint` mode (Steps 1 + 2 + Friction Enumeration + Persist + Persist-Pending-Tasks); end-of-session uses full retrospective. Aim for 3-5 wraps per long working session - dense enough to catch learnings, not so dense that wrap overhead dominates. `/clear` is a CLI-side command only the user can execute; Claude can invoke the wrap skill via the `Skill` tool but cannot trigger `/clear` itself.
- **Mid-task context reset fallback when you can't yet wrap.** When context bloats but you're not at a checkpoint (e.g. 80% through a feature, can't yet wrap-and-clear, output quality already degrading): copy critical state from the terminal → `/compact` for a synthesized summary → `/clear` for fresh context → paste back ONLY what matters. Manual ritual; the compaction step seeds Claude with the synthesized summary, the clear-and-paste step ensures the new context starts clean rather than carrying compaction artifacts. Use when wrap-and-clear isn't yet feasible mid-task.
- **Use `#` to capture corrections as durable rules in-session.** Pressing `#` while working triggers Claude to add the surrounding context as an instruction to CLAUDE.md - the lowest-friction way to memorialize a correction-in-the-moment. For per-project CLAUDE.md files this writes to the project's CLAUDE.md directly. For global `~/.claude/CLAUDE.md`, prefer editing the dotfiles source so the change syncs across machines; if you do use `#` on the live file, re-add to chezmoi afterwards. Pair with periodic Skill Health Audit Q11 (instruction count) so accumulated `#` captures don't bloat the instruction budget.
...
**Authoritative source for the "all registered" set.** Determine which persistent-memory MCPs are registered from `~/.claude.json` `mcpServers` keys or `claude mcp list` output - NOT from the session's deferred-tool list. Tool-list absence means "not connected this session", not "not registered": a registered MCP that failed to connect at session-start has no tools in the list, which silently looks identical to never-registered. When a registered persistent-memory MCP shows `Failed to connect`, surface the gap explicitly before firing single-writer fallback; do not degrade silently to fewer writers. The user decides whether to single-write-and-log-gap or fix-and-retry. (Most fixes require a full Claude Code restart since the MCP set is frozen at session-start - `/clear` alone does not re-register MCPs.)
